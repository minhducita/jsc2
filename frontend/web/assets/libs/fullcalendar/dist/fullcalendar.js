...
var numberWeek = 0;
var $activeWeek = 0;
var setDateStart = function(cell) {
	
	var weekNow = getWeekNumber(new Date());
		weekNow[1] -= 1; 
	var weekCell = getWeekNumber(new Date(cell.start._d));
	
	if(((weekNow > weekCell && $activeWeek == 0) || $activeWeek == 1) && weekNow[0] == weekCell[0]) {
		$activeWeek = 1;
		if(numberWeek == 0) {
			numberWeek = (weekNow[1] - weekCell[1])*7 ;
		} 
		var dateCell =  new Date(cell.start.format());
		dateCell.setDate(dateCell.getDate() + numberWeek);
		if(activeButtonWeek != 0) {
			dateCell.setDate(dateCell.getDate() + (activeButtonWeek * 7));
		}
		
		var date = moment(dateCell);
	} else {
		var date = cell.start;
	}
...
var setDateEnd =  function(cell) {
	var weekNow = getWeekNumber(new Date());
		weekNow[1] -= 1; 
	var weekCell = getWeekNumber(new Date(cell.end._d));
	...
	if(((weekNow > weekCell && $activeWeek == 0) || $activeWeek == 1) && weekNow[0] == weekCell[0]) {
		...
		var dateCell =  new Date(cell.end.format());
		dateCell.setDate(dateCell.getDate() + numberWeek);
		if(activeButtonWeek != 0) {
			dateCell.setDate(dateCell.getDate() + (activeButtonWeek * 7));
		}
...
var Grid = fc.Grid = RowRenderer.extend({
	...
	setRange: function(range) {
		var start = setDateStart(range);
		var end   = setDateEnd(range);
		angular.extend(range.start, start);
		angular.extend(range.end, end)
		var view = this.view;
...
function Calendar_constructor(element, overrides) {
	...
	function renderView(viewType, week) {
		ignoreWindowResize++;
		...
		if (currentView) {
			...
			if (
				...
				angular.isDefined(week)
			) {
				if (elementVisible()) {
					
					freezeContentHeight();
				
					currentView.display(date, week);
...
Calendar.defaults = {
	...
	buttonIcons: {
		...
		prevWeek: 'left-single-arrow',
		nextWeek: 'right-single-arrow',
...
Calendar.rtlDefaults = { // right-to-left defaults
	...
	buttonIcons: {
		...
		prevYear: 'left-double-arrow',
		nextYear: 'right-double-arrow',
		prevWeek: 'left-double-arrow',
		nextWeek: 'right-double-arrow',
	},
	themeButtonIcons: {
		...
		if (buttonStr) {
			$.each(buttonStr.split(' '), function(i) {
				...
				if (groupChildren.length > 1) {
					...
					var eleDiv = "";
					var elep = "";
					angular.forEach(groupChildren, function($child, $key) {
						if(flag < 2) {
							if(flag %2 == 0) {
								elep = document.createElement('p');
								elep.appendChild(document.createTextNode('Month'));
								elep.className = 'buttonMonth';
								
								eleDiv = document.createElement('div');
								eleDiv.appendChild(elep);		
								eleDiv.className = 'groupButtonMonth';
								eleDiv.appendChild($child);
							}  else {
								eleDiv.appendChild($child);
								divchild.push(eleDiv);
							}
							flag++;
						} else if (flag < 4) {
							if(flag %2 == 0) {
								elep = document.createElement('p');
								elep.appendChild(document.createTextNode('Week'));
								elep.className = 'buttonWeek';
								
								eleDiv = document.createElement('div');
								eleDiv.appendChild(elep);		
								eleDiv.className = 'groupButtonWeek';
								eleDiv.appendChild($child);
							}  else {
								eleDiv.appendChild($child);
								divchild.push(eleDiv);
