 ...
 */
app.controller('BoardController', ['$rootScope', '$scope', 'AppService', 'Socket', 'toaster', 'activitys', '$filter', 'MEMBER_ROLES', 'Upload', '$timeout', 'ngMeta', '$location', 'orderByFilter', 'Notification', '$templateRequest', '$sce', '$compile', function(a, b, c, d, e, f, g, h, i, k, m, n, o, l, temp, sce, compile) {
    b.listHour = '';
    b.boardPageAssets = [];
    b.prefsBackground  = [
    ...
    a.$on('E:BOARD_GETTED', function(event, data) {
        ...
        /*end meta tags*/
        a.configs = {
            backgroundType: data.prefs['backgroundType'],
        ...
        angular.forEach(b.boardPageAssets.lists, function(list) {
            if(list.sort != 0)
            {
                list.cards = showListSortPriority( _.where(b.boardPageAssets.cards, {idList: list.id}) );
            }else
    ...
    /**
     * Get notification for board and CARD;
     **/
    ...
    /**
     * desc: change background of Board;
     **/
    d.on('WE:BOARD_BACKGROUND_CHANGED', function(data) {
        ...
        if (angular.isDefined(prefs)) {
            ...
            } else if(prefs.backgroundType == "picture"){
                a.configs = {
    ...
    /**
     * Desc: add member for Board
     **/
    d.on('WE:ADDED_MEMBER_TO_BOARD', function(data) {
    ...
    /**
     * desc: remove file attachment in card
     **/
    ...
    /**
     * desc: remove member in Board
     **/
    d.on('WE:REMOVED_MEMBER_OF_BOARD', function(data) {
        if(data.idBoard == b.boardPageAssets.id) {
            var idMember = data.idMember;
            if (angular.isDefined(b.boardPageAssets.members)) {
    ...
    /**
     * desc: change permissions member in Board
     **/
    d.on('WE:BOARD_MEMBER_ROLE_CHANGED', function(data) {
        if(data[2] == b.boardPageAssets.id) {
    ...
    /**
     * desc: socket create list for board
     **/
    ...
    /**
     * desc: socket move list in board
     **/
    ...
    /**
     * desc: socket update list for board
     **/
    ...
    /**
     * socket update all card, when copy or move list;
     **/
    ...
    /**
     * socket update checklist in card
     */
    ...
    /**
     * socket delete checklist in card
     */
    ...
    /**
     * socket add checklist item for checklist in card
     */
    ...
    /**
     * socket update checklist item in card
     */
    ...
    /**
     * socket delete checklist item in card
     */
    ...
    /**
     * socket update comment for card
     */
    ...
    /**
     * socket delete comment for card
     */
    ...
    /**
     * socket add file (function attachment) for card
     */
    ...
    /**
     * socket list change background color;
     */
     ...
     * @param model
     */
    b.submitReport = function(input, badgesDue) {
        idMember = a.appAssets.id;
        if (input.hour != undefined  && badgesDue != undefined) {
            var date = new Date(badgesDue);
            c.boards.addReport(input.hour, idMember, b.cardDetail.id, b.boardPageAssets.id, date)
                .then(response => {
                    if (!response.status) {
                        alert(response.message);
                    } else {
                        c.boards.getHour(b.cardDetail.id)
                            .then(response => {
                                b.listHour = response;
                            });
                        alert(response.message);
    ...
    b.changeBackground = function(model) {
        ...
        return !b.hasMemberInBoard(a.appAssets.id) ? void 0 : c.boards.update(b.boardPageAssets.id, model).then(function(response) {
            if( model.backgroundType === "picture" && !angular.isUndefined(model.backgroundImage) ){
                a.configs = {
    ...
    b.changeMemberRole = function(idMember, role) {
        ...
        return c.boards.model = b.boardPageAssets, ms = b.countByRoleMemberShips(), r = g('memberRoles')(a.appAssets.id, b.boardPageAssets.memberShips), r === 'Admin' && angular.isDefined(ms.admin) && ms.admin < 2 && idMember == a.appAssets.id ? void 0 : (r === 'Admin' ? mr = angular.lowercase(g('memberRoles')(idMember, b.boardPageAssets.memberShips)) !== role ? c.boards.changeRole(idMember, role).then(function(response) {
            ...
            d.emit('WE:BOARD_MEMBER_ROLE_CHANGED', [idMember, role, b.boardPageAssets.id]);
            /*notification*/
            getNotification();
        }): void 0: void 0);
    };
    ...
    b.createCard = function(model) {
        ...
        if ((typeof model.important !== 'undefined')) {
            model.important = model.important == 1 ? 1 : 0;
        }
        if ((typeof model.urgent !== 'undefined')) {
            model.urgent = model.urgent == 1 ? 1 : 0;
        }
    ...
    b.updateCard = function(idCard, model) {
        ...
        /** Priority -- Minhquyen **/
        if(model.important != undefined) {
            model.important = model.important == 1 ? 0 : 1;
        }
        if(model.urgent != undefined) {
            model.urgent = model.urgent == 1 ? 0 : 1;
        }
        /** end Priority **/
    ...
    b.getCardDetail = function(card) {
        ...
        c.cards.getCard(card.id).then(function(response) {
            ...
            c.boards.getHour(card.id)
                .then(response => { b.listHour = response; });
            card.attachmentSelected = _.find(card.attachments, {id: card.idAttachmentCover});
            b.cardDetail  = card;
            c.cards.model = card;
            ...
            angular.forEach(b.cardDetail.checklists, function(value, key) {
                value.checkItems = _.map(value.checkItems, function(v, k)
                {
                    v.due = (v.due == 0 || v.due=="" || angular.isUndefined(v.due))  ? undefined : parseInt(v.due);
                    return v;
                });
            });
     ...
     */
    var event_createLabel = 0;
    b.cardCreateLabel = function(name, color) {
        var l;
        if(event_createLabel == 0) {
            event_createLabel = 1;
            (l = _.find(b.boardPageAssets.labels, function(l) {
                ...
                $('.popover-create-label.popup-child').hide();
            });
        }
    };
    ...
    /*** @minhquyen ***/
    b.give_buff_card = function (card) {
        b.cardDetail = card;
        c.cards.model = card;
        b.idMemberActiveCard = _.pluck(b.cardDetail.members, 'id');
        b.cardDetail.checklists = [];
     ...
     * @returns {*}
     */
    b.cardAddChecklistItems = function(idChecklist, checklistItem) {
        if(angular.isDefined(checklistItem.due)) {
            var date = new Date(checklistItem.due);
            checklistItem.due = date.getTime();

        }

        return checklistItem.name === '' || !b.hasMemberInBoard(a.appAssets.id) ? void 0 : c.cards.addChecklistItems(idChecklist, checklistItem).then(function(response) {
            var currentChecklist = _.find(b.cardDetail.checklists, {id: idChecklist});
            if (angular.isDefined(currentChecklist)) {
    ...
    b.cardUpdateChecklistItems = function(idChecklistItem, model) {
        if(angular.isDefined(model.due)) {
            var date = new Date(model.due);
            model.due = date.getTime();
        }
    ...
    b.getChecklistProgress = function(checklist) {
        ...
        if(angular.isUndefined(checklist.checkItems)) {
    ...
    /* event click card , card seard header*/
    a.$on('E:EVENT_CLICK_FILTER_CARD', function(event, data) {
        if(angular.isDefined(b.boardPageAssets.cards)) {
            var listIdCard = _.pluck(b.boardPageAssets.cards, 'id');
            if(_.contains(listIdCard, parseInt(data))) {
                _.defer(function() {
                    $('div[cardid='+data+'] > div').trigger('click');
                })
            }
        }
    })
    ...
    b.filterCardRelationShip = function(parentId) {

        b.filterCard['relation'] = [];
        if(angular.isDefined(parentId) && parentId.length > 0) {
            b.filterCard['relation'] = parentId;
            b.filterCard['relation'].push(b.cardDetail.id);
            _.defer(function() {
                $(".close").trigger('click');
                $('.scrollbarchange').perfectScrollbar('update');
            });
    ...
    b.colorDue = function(card, $type) {
        ...
        var $dateCardDeadline = 0;
        if($type == 'due') {
            $dateCardDeadline  = card.badges.due;
        }
        else if($type=='startDate') {
            $dateCardDeadline  = card.badges.startDate;
        }
        else if($type=='checklistItem') {
            $dateCardDeadline  = card.due;
        }
    ...
    /** --TA--
     * ANOTHER BOARD
     * param: listId, model, listdetail
     * => service.js
     **/
    b.updateCardAnotherBoard = function (listId, model, listdetail) {
        if (model.idList && listId !== model.idList) {
            return !b.hasMemberInBoard(a.appAssets.id) ? void 0 : c.cards.updateCardAnotherBoard(listId, model, listdetail).then(function (response) {
                getUpdateCard(response);
                _.defer(function () {
                    $('.modal-close').trigger('click');
                });
        ...
        _.defer(function () {
            $('.modal-close').trigger('click');
            $(".CheckMoveCard").prop("checked", false);
        });
    };

    /* --TA--
    ng-init trong list-action.html
    */
    b.anotherGetList = function (list) {
        b.listPresent = list;//gán $scope.listPresent vào Scope sủ dụng trong cho Directive 'another'
    };
     ...
     **/
    var check_cardId = [];
    b.checkboxOption = function (event, cardId) {
        var action = (event.target.checked ? 'add' : 'remove');
        if (action === 'add') check_cardId.push(cardId);
        if (action === 'remove') {
            var index = check_cardId.indexOf(cardId);
            check_cardId.splice(index, 1)
        }
        return check_cardId;
    };
    ...
    /** --TA--
     * CARD SELECTED MOVE
     * param: $idList, model 'idchange', arr_check === arr_check_card, listDetailCards (object card)
     * => service.js
     **/
    b.updateCardSelected = function ($idList, model, listDetailCards) {
        //get cardId present in object when click button "List Action"
        var present_cardId = [];
        for (var Card in listDetailCards) {
            var cardId = listDetailCards[Card].id;
            present_cardId.push(cardId);
        }
        //compare  compare 2 array
        var choosed_cardId = intersect_arrays(check_cardId, present_cardId);
        if (choosed_cardId.length > 0) {
            return !b.hasMemberInBoard(a.appAssets.id) ? void 0 : c.cards.updateCardSelected($idList, model, choosed_cardId).then(function (response) {
                getUpdateCard(response);
                _.defer(function () {
                    $('.modal-close').trigger('click');
                });
    ...
    /* --TA--
     *   compare 2 array => intersect array value
     *   array1 = [1,2,3,4]
     *   array2= [2,4,5]
     *   intersect_arrays = (array1, array2) ==> [2,4]
    */
    ...
    /**update sort for list || MINHQUYEN **/
    b.sortByPriority = function (idList, model) {

        model.sort = model.sort == 0 ? 1 : 0;

        return !b.hasMemberInBoard(a.appAssets.id) ? void 0 : c.lists.update(idList, model).then(function (response) {
            var curentList = _.find(b.boardPageAssets.lists, {id: idList});
            curentList = angular.extend(curentList, response);
            ...
            angular.forEach(b.boardPageAssets.lists, function (list, key) {
                if (list.id == response.id) {
                    b.boardPageAssets.lists[key] = angular.extend(b.boardPageAssets.lists[key], response);
                    var currentCard = b.boardPageAssets.lists[key].cards;

                    //update priority card -- Minhquyen
                    if(b.boardPageAssets.lists[key].sort != 0){
                        b.boardPageAssets.lists[key].cards = angular.extend(b.boardPageAssets.lists[key].cards, showListSortPriority( b.boardPageAssets.lists[key].cards ));
                    }else if(b.boardPageAssets.lists[key].sort == 0) {
                        b.boardPageAssets.lists[key].cards = angular.extend(b.boardPageAssets.lists[key].cards, _.where(b.boardPageAssets.cards, {idList: idList}));
                    }
                    //end
    ...
    // show list for PRIORITY - Minhquyen
    var showListSortPriority = function( listCard ) {
        angular.forEach(listCard, function($card){
            if($card.important == 1 && $card.urgent == 1){
                $card.sortIndex = 1;
            };
            if($card.important == 1 && $card.urgent == 0){
                $card.sortIndex = 2;
            };
            if($card.important == 0 && $card.urgent == 1){
                $card.sortIndex = 3;
            };
            if($card.important == 0 && $card.urgent == 0){
                $card.sortIndex = 4;
            };
        });

        return _.sortBy(listCard, 'sortIndex');
    };
    /** bind event show card on Priority popup **/
    b.eventOpenCard = function (cardData) {
        if(cardData){

            var templateUrl = sce.getTrustedResourceUrl('app-clients/widgets/popup/card-detail/card-detail-inner.tpl.html');
            b.card  = cardData;
            temp(templateUrl).then(function(template) {
                compile($(".popup-card-detail").html(template).contents())(b);
            }, function() {
    ...
    b.getList = function (listId) {
        $listDetail = _.find(b.boardPageAssets.lists, {id: parseInt(listId)});
        b.listDetail = $listDetail;
    };

    b.eventOpenList = function (idList) {

        var templateUrl = sce.getTrustedResourceUrl('app-clients/widgets/popup/lists/list-priority-detail.tpl.html');
        b.listid = idList;
        temp(templateUrl).then(function(template) {
            compile($(".popup-show-priority").html(template).contents())(b);
        }, function() {
            // An error has occurred
        });
